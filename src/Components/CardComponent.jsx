    ...
    useEffect(()=>{
      const interval =  setInterval(()=>{
            setImageIndex(prevIndex=>{
                const totalImages = roomData[0]?.images.length || 1; 
      ...
      <div className={activeItem ? 'itemDetailDiv' :'ContainerDiv'}>
        {
            activeItem ? <ActiveComponent activeItem={activeItem} onBackClick={handleBackClick}/> 
                : (
                    ...
                    .map(item =>
                        (
                            <div className='cardComponentMainDIv' key={item.id} onClick={()=>handleNewContent(item)} onMouseEnter={()=>setHoverId(item.id)} onMouseLeave={()=>setHoverId(null)} >
                                {/* {hoverId ===item.id && <div className='hover-div'>
                                    {<img src={item.images[0]} alt="" className='hoverImage'/>}            
                                </div>} */}
